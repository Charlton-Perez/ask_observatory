...
export default async function handler(req) {
  ...
  if (exceedanceSlice)
    dataContext.push(`On-demand threshold computation for this query (computed from full daily record in the browser):\n${JSON.stringify(exceedanceSlice)}\nIMPORTANT: The 'currentPeriod.count' value is the authoritative answer — use it directly, do NOT recount from other data. Thresholds are inclusive (≥ for warm/wet, < for cold/frost). If 'matchingDays' is present, list those dates and values in your answer. Use 'byMonth' to identify which months the events fell in — do not attribute events to the current calendar month if byMonth shows they occurred in a different month. Only compare against monthly historical stats for the month(s) shown in byMonth.`)
  if (etccdiSlice)
